...
class convMESH():
    ...
    def __init__(self, datainfo):
        ...
        if advance_api:
            ...
            # dataset = dataset.map(...)
            if fast:
                dataset_train = dataset_train.repeat(self.maxepoch_deform+self.maxepoch_structure).shuffle(buffer_size = 10000).batch(self.batch_size)
                dataset_valid = dataset_valid.repeat(self.maxepoch_deform+self.maxepoch_structure).shuffle(buffer_size = 10000).batch(self.batch_size)
...
#------------------------------------------------------------training function------------------------------------------------------------------------------------
    ...
    def train_total_deform_structurev1(self):# trian with split the dataset to test the generalization error
        ...
        All_epoch = self.maxepoch_deform+self.maxepoch_structure
        for epoch in range(checkpoint_counter, All_epoch):
            ...
            printout(self.flog,"Train Epoch: {:6d}|{:6d} gener_loss: {:08.4f} weight_loss: {:08.4f} kllll_loss: {:08.4f} l2222_loss: {:08.4f}".format(epoch, cost_generation, weights_norm, cost_kl, cost_l2), epoch)
    ...
    def train_total_deform_structurev2(self):# trian with split the dataset to test the generalization error % short time
        ...
        train_feature = self.feature[self.train_id]
        train_symmetry_feature = self.symmetry_feature[self.train_id]
        train_feature, train_symmetry_feature, epoch_train = get_batch_data1(train_feature, train_symmetry_feature, All_epoch, self.batch_size)
        dictbase_train = {self.inputs_feature: train_feature, self.inputs_symmetry: train_symmetry_feature}
        random_train = np.zeros((self.batch_size,200)).astype('float32')

        valid_feature = self.feature[self.valid_id]
        valid_symmetry_feature = self.symmetry_feature[self.valid_id]
        valid_feature, valid_symmetry_feature, epoch_valid = get_batch_data1(valid_feature, valid_symmetry_feature, All_epoch, self.batch_size)
        dictbase_valid = {self.inputs_feature: valid_feature, self.inputs_symmetry: valid_symmetry_feature}
        random_valid = np.zeros((self.batch_size,200)).astype('float32')
        ...
        All_epoch = max(epoch_train, epoch_valid)
        for epoch in range(checkpoint_counter, All_epoch):
            if not epoch<epoch_train:
                self.sess.run(self.train_iterator.initializer, feed_dict=dictbase_train)
            if not epoch<epoch_valid:
                self.sess.run(self.valid_iterator.initializer, feed_dict=dictbase_valid)
            printout(self.flog, self.iddat_name, epoch, interval=100, write_to_file = False)
            ...
            printout(self.flog,"Train Epoch: {:6d}|{:6d} gener_loss: {:08.4f} weight_loss: {:08.4f} kllll_loss: {:08.4f} l2222_loss: {:08.4f}".format(epoch, All_epoch, cost_generation, weights_norm, cost_kl, cost_l2), epoch)
    ...
    def train_deform_structure_seperatev1(self): # advance api
        printout(self.flog,"Train Structure Net...")
        inf = float('inf')
        tf.global_variables_initializer().run()
        # tf.local_variables_initializer().run()
        ...
        valid_handle = self.sess.run(self.valid_iterator.string_handle())
        All_epoch = self.maxepoch_deform+self.maxepoch_structure
        for epoch in range(checkpoint_counter_struture, All_epoch):
            ...
            if epoch < self.maxepoch_deform:
                ...
                printout(self.flog,"Train Epoch: {:6d}|{:6d} \nPart  Name: {} \ngener_loss: {} \nweigh_loss: {} \nkllll_loss: {} \nl2222_loss: {}".format(epoch, All_epoch, self.part_name, cost_generation, weights_norm, cost_kl, cost_l2), epoch)
                ...
                if len(valid_feature)>0:
                    ...
                    printout(self.flog,"\nPart  Name: {} \nvalid_Gen_loss: {} \nvalid_kl_loss: {}".format(self.part_name, cost_generation_valid, cost_kl_valid),epoch)
                ...
                printout(self.flog,"Epoch: {:6d}|{:6d} gener_loss: {:08.4f} kllll_loss: {:08.4f} l2222_loss: {:08.4f}".format(epoch, All_epoch, cost_generation, cost_kl, cost_l2),epoch)
    ...
    def train_deform_structure_seperatev2(self): # advance api
        printout(self.flog,"Train Structure Net...")
        inf = float('inf')
        tf.global_variables_initializer().run()
        # tf.local_variables_initializer().run()
        ...
        for epoch in range(checkpoint_counter_struture, ALL_epoch):
            if not epoch<epoch_train:
                self.sess.run(self.train_iterator.initializer, feed_dict=dictbase_train)
            if len(self.valid_id)>0 and not epoch<epoch_valid:
                self.sess.run(self.valid_iterator.initializer, feed_dict=dictbase_valid)

            lowvalue = float('inf')
            printout(self.flog, self.iddat_name, epoch, interval=100, write_to_file = False)
            if epoch < int(max(epoch_train, epoch_valid)*0.25):
                if self.ae:
                    dictrand = {x: random_train[:, 0: np.shape(x)[1]] for x in self.embedding_inputs[:-1]}
                    feed_dict_train = merge_two_dicts(dictrand, {self.handle: train_handle})
                else:
    ...
    def train_scvae(self):
        ...
        if advance_api:
            with tf.Session(config = self.config) as self.sess:
                if self.union:
                    if fast:
                        self.train_total_deform_structurev2()
                    else:
                        self.train_total_deform_structurev1()
                else:
                    if fast:
                        self.train_deform_structure_seperatev2()
                    else:
                        self.train_deform_structure_seperatev1()
...
#------------------------------------------------------------applications--------------------------------------------------------------------------------------
    ...
    def generateshapebaseonstructure(self, datainfo, id, epoch = 0):
        with tf.Session(config = self.config) as sess:
            tf.global_variables_initializer().run()
            if epoch == 0:
                _success, epoch = self.load(sess, self.checkpoint_dir_structure)
            ...
            if not _success:
                raise Exception("抛出一个异常")
            path = self.checkpoint_dir_structure +'/../genbasesamestruc'+str(epoch)
            if not os.path.isdir(path):
                os.makedirs(path)
            # print(np.expand_dims([i for i in datainfo.modelname], axis=0))
            for i in range(len(id)):
                if isinstance(id[i], str) and len(id[i])>len(str(datainfo.modelnum)):
                    id[i] = datainfo.modelname.index(id[i])
                else:
                    id[i] = int(id[i])
                print('ID: {:3d} Name: {}'.format(id[i], datainfo.modelname[id[i]]))
            ...
            for i in range(self.part_num+1):
                ...
                if i < self.part_num:
                    ...
                    deforma_v1, deforma_v_align = sess.run([self.deform_vertex[i], self.deform_vertex_align[i]], feed_dict={self.feature2point: recover, self.controlpoint: np.tile([0,0,0], (np.shape(recover)[0], 1, 1))})
                    # deforma_v2, deforma_v_align = sess.run([self.deform_vertex, self.deform_vertex_align], feed_dict={self.feature2point: recover2, self.controlpoint: np.tile([0,0,0], (np.shape(recover2)[0], 1, 1))})
                    deforma_v1 = self.alignallmodels(deforma_v1, id = i, modelid = inter_id[0])
                    # deforma_v2 = self.alignallmodels(deforma_v2, one=False)
                    # deforma_v_align = alignallmodels(deforma_v_align)
                    num_list_new = [str(x) for x in np.arange(np.shape(deforma_v1)[0])]
                    self.v2objfile(deforma_v1, path + '/' + self.part_name[i], num_list_new, num_list_new, self.part_name[i])
                    ...
                    for k in range(self.part_num):
                        ...
                        # recover1, emb, std = sess.run([self.test_decode[i], self.encode[i], self.encode_std[i]], feed_dict = feed_dict)
                        deforma_v1, deforma_v_align = sess.run([self.deform_vertex[k], self.deform_vertex_align[k]], feed_dict={self.feature2point: recover1, self.controlpoint: np.tile([0,0,0], (np.shape(recover1)[0], 1, 1))})
                        deforma_v1 = self.alignallmodels(deforma_v1, id = k, modelid = inter_id[0])
                        num_list_new = [str(x) for x in np.arange(np.shape(deforma_v1)[0])]
                        self.v2objfile(deforma_v1, path + '/struc_' + self.part_name[k], num_list_new, num_list_new, self.part_name[k])
...
#------------------------------------------------------------feature2vertex--------------------------------------------------------------------------------------
    ...
    def alignallmodels(self, deforma_v, id = 0, one = True, modelid = 1):
